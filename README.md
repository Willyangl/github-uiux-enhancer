...
GitHub Actions のワークフロー一覧で、ブランチ名が省略されて見えない問題を解消します。列幅に合わせて自動改行し、全文を表示します。

### 3. ブランチ名のコピーボタン
GitHub Actions のワークフロー履歴の各行に、ブランチ名をコピーするボタンを追加します。クリックするとクリップボードにコピーされ、「Copied!」ツールチップが表示されます。

### 4. ワークフロー完了通知
- **一覧画面**: 実行中ワークフロー行に「通知」ボタンを表示。クリックで通知登録。
- **詳細画面**: ワークフロー実行直後のリダイレクト先でも通知ボタンを表示。
...
トークンはブラウザの `chrome.storage.local` に保存され、GitHub API へのリクエスト認証に使用されます。
...
├── content.js          # コンテンツスクリプト（機能1〜4のDOM操作）
...
- バックグラウンドワーカーは1分間隔でGitHub APIをポーリングします。API Rate Limit（5,000回/時）にご注意ください。
